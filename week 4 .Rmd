---
title: "Bayesian Linear Regression week 4 lab"
author: "Michelle Tan"
date: "6/21/2018"
...
```{r two-vars-eda}
ggplot(wage, aes(x=hours, y=wage))+geom_point()

ggplot(wage, aes(x=educ, y=wage))+geom_point()
...
```{r}
# Checking assumption of errors being normal distribution of residudals with a constant variance
...
The increase percent of wage per 1 standard deviation increase in IQ is approximately 0.3336.
</div>
```{r z-iq}
...
# when "ask" argument is set to "FALSE", it doesn't ask for "return" for plot # 
```
...
```{r}
sort(summary(bma_lwage_red)[1:12, 1], decreasing = FALSE)
```
...
```{r}
# BPM reduced #
BPM_pred_lwage_red <- predict(bma_lwage_red, estimator = "BPM")
BPM_cov <- bma_lwage_red$namesx[BPM_pred_lwage_red$bestmodel+1]

# MPM reduced #
MPM_pred_lwage_red <- predict(bma_lwage_red, estimator = "MPM")
MPM_cov <- bma_lwage_red$namesx[MPM_pred_lwage_red$bestmodel+1]

# HPM reduced #
HPM_pred_lwage_red <- predict(bma_lwage_red, estimator = "HPM")

HPM_cov <- bma_lwage_red$namesx[HPM_pred_lwage_red$bestmodel+1]

BPM_cov; MPM_cov; HPM_cov
```
